 ...
 * 00h - 07h bank0
 * 18h - 1Fh bank1
 * 08h 17h is stack ( 12bits of adresse and CY, AC, F0 and BS flags )
 *
 * 0000 0003 00007 is reserved adress
 *
 * */
...
void cpu_exec()
{
	...
	for (;;) {
		...
		lastpc = pc;
		op = ROM(pc);
		pc = (pc & 0xF800) | ((pc+1) & 0x07FF);

		/*printf("DEBUG %s() %04x %02x %02x r%02x ", __func__, pc-1, op, ROM(pc), reg_pnt);*/
/*		for(temp = 0 ; temp < 32; temp++)
			printf(" %02x", intRAM[temp]);
		printf("\n");*/
		switch (op) {
			case 0x00: /* NOP */
				clk++;
			...
			case 0x03: /* ADD A,#data */
				...
				cy=ac=0;
				dat=ROM(pc++);
				if (((acc & 0x0f) + (dat & 0x0f)) > 0x0f) ac=0x40;
				temp=acc+dat;
				if (temp > 0xFF) cy=1;
				acc=(temp & 0xFF);
				break;
			...
			case 0x13: /* ADDC A,#data */
				...
				dat=ROM(pc++);
				ac=0;
				if (((acc & 0x0f) + (dat & 0x0f) + cy) > 0x0f) ac=0x40;
				temp=acc+dat+cy;
				cy=0;
				if (temp > 0xFF) cy=1;
				acc=(temp & 0xFF);
		...
		if ((mstate == 1) && (master_clk > evblclk)) {
			handle_evbl();
			/* TODO why this 2nd function ? */
			/*if (app_data.crc == 0xA7344D1F)*/
			if (app_data.handleevbll == 1)
