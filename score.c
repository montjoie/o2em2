...
void save_highscore(int highscore, char *scorefile)
{
	...
	if (scorefile == NULL) {
		fprintf(stderr, "%s scorefile is NULL\n", __func__);
		return ;
	}
	printf("%s %d on %s\n", __func__, highscore, scorefile);
	highscore = highscore==app_data.default_highscore?0:highscore;

        fn = fopen(scorefile, "w");
	if (fn == NULL) {
		fprintf(stderr, "Error opening highscore-file %s: %s\n", scorefile, strerror(errno));
		o2em_clean_quit(EXIT_FAILURE);
	}

	if (fprintf(fn, "%i", highscore) <= 0) {
		fclose(fn);
		fprintf(stderr, "Error writing to highscore-file %s: %s\n", scorefile, strerror(errno));
		o2em_clean_quit(EXIT_FAILURE);
	}
	fclose(fn);
}
