...
void vpp_finish_bmp(Byte *vmem, int offx, int offy, int w, int h, int totw, int toth)
{
	...
	for (y = 0; y < h; y++){
		...
		#else
		pnt2 = get_raw_pixel_line(vppbmp, y);
		#endif
...
static void vpp_draw_char(int x, int y, Byte ch, Byte c0, Byte c1, Byte ext, Byte dw, Byte dh, Byte ul){
	...
	for (yy=0; yy<10; yy++) {
		...
		m = (dw==2) ? 0x08 : 0x80;
		#ifdef __O2EM_SDL__
		pnt = get_raw_pixel_line(vppbmp, y * 10 + yy) + x * 8;
		#endif
		for (xx = 0; xx < 8; xx++) {
			#ifndef __O2EM_SDL__
...
static void vpp_update_screen()
{
	...
	for (y = 0; y < 25; y++) {
		...
		/* for every character */
		for (x = 0; x < 40; x++) {
			hpar = (lhd==0) ? 0 : 1-hpar; /* horizontal double size alternates between 1 and 0 */

			/* get parallel attribute */
			/*chr = vpp_mem[x][l][0]; commented in manopac sources */
			attr = vpp_mem[x][l][1];

			c1 = attr & 0x7; /* color */
			c1 = ((c1&2) | ((c1&1)<<2) | ((c1&4)>>2)); /* invert the bits (0=2, 1=1,2 =0) */
			ext = (attr & 0x80) ? 1 : 0; /* extended characters ? col_patr_blck*/

			/* get serial character and serial attribute */
			ser_chr = vpp_mem[x][l][2];
			ser_atr = vpp_mem[x][l][3];
			if (ser_chr) { /* serial character, so look at the serial attributes */
				c0 = (ser_atr>>4) & 0x7; /* background color */
				c0 = ((c0&2) | ((c0&1)<<2) | ((c0&4)>>2)); /* invert bits */
				ul = ser_chr & 4; /* underlined */
				conc = ser_chr & 1; /* concealed */
				box = ser_chr & 2; /* boxed mode */
			} else
				if (ext) {
					/* extended character */
					c0 = (attr>>4) & 0x7; /* so bits 3-6 are background color */
					c0 = ((c0&2) | ((c0&1)<<2) | ((c0&4)>>2)); /* invert bits */
					dw = dh = 0; /* no double size characters possible for extended */
				} else {
					dw = (attr & 0x20) ? (hpar ? 2 : 1) : 0; /* if bit set -> dw is either 1 or 2 */
					dh = (attr & 0x10) ? (vpar ? 2 : 1) : 0; /* if bit set -> either 1 or 2 */
					if (dw) lhd = 1; /* turn lhd on*/
					if (dh) lvd = 1; /* turn lvd on*/
				}
	...
	if (vpp_r & 0x20) {
		for (y = vppbmp->h - 1; y >= 10; y--)
			...
			#else
			p_dst = get_raw_pixel_line(vppbmp, y);
			p_src = get_raw_pixel_line(vppbmp, (y - 10) / 2 + 10);
			for (x = 0; x < vppbmp->w; x++)
				p_dst[x] = p_src[x];
			#endif
	}
