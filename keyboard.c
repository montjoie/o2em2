...
/*===========================================================================*/
/*===========================================================================*/
/* change key binding for joystick*/
...
{
		set_systemkeys(KEY_F12, KEY_F1, KEY_F4, KEY_F5, KEY_F8, KEY_F2, KEY_F3, KEY_F6, KEY_F7, KEY_F9, KEY_F10, KEY_F11);
}
...
void swap_joykeys()
{
	...
	for (i = 0; i < 5; i++) {
		key = joykeys[0][i];
		joykeys[0][i] = joykeys[1][i];
		joykeys[1][i] = key;
	}
	key = app_data.sticknumber[0];
	app_data.sticknumber[0] = app_data.sticknumber[1];
	app_data.sticknumber[1] = key;

	key = app_data.stick[0];
	app_data.stick[0] = app_data.stick[1];
	app_data.stick[1] = key;
}
...
void set_joykeys(int jn, int up, int down, int left, int right, int fire)
{
	int i, j;
	if ((jn < 0) || (jn > 1)) {
		fprintf(stderr, "%s ERROR invalid player number %d\n", __func__, jn);
		return;
	}
	joykeys[jn][0] = up;
	joykeys[jn][1] = down;
	...
	for (j = 0; j < 2; j++)
		for (i = 0; i < 5; i++) {
			#ifndef __O2EM_SDL__
			/* TODO SDL have keycode > 127 put KEY_MAX ?*/
...
{
	/* SDL or allegro arent init so calls to SDL_GetKeyName or scancode_to_name fails
	#ifdef __O2EM_KEYBOARD_DEBUG__
	#ifdef __O2EM_SDL__
	printf("%s Quit is %d %s\n", __func__, k_quit, SDL_GetKeyName(k_quit));
	printf("%s Pause is %d %s\n", __func__, k_pause, SDL_GetKeyName(k_pause));
	printf("%s debug is %d %s\n", __func__, k_debug, SDL_GetKeyName(k_debug));
	printf("%s reset is %d %s\n", __func__, k_reset, SDL_GetKeyName(k_reset));
	printf("%s screenshot is %d %s\n", __func__, k_screencap, SDL_GetKeyName(k_screencap));
	printf("%s save is %d %s\n", __func__, k_save, SDL_GetKeyName(k_save));
	printf("%s load is %d %s\n", __func__, k_load, SDL_GetKeyName(k_load));
	printf("%s inject is %d %s\n", __func__, k_inject, SDL_GetKeyName(k_inject));
	#else
	printf("%s Quit is %d %s\n", __func__, k_quit, scancode_to_name(k_quit));
	printf("%s Pause is %d %s\n", __func__, k_pause, scancode_to_name(k_pause));
	printf("%s debug is %d %s\n", __func__, k_debug, scancode_to_name(k_debug));
	printf("%s reset is %d %s\n", __func__, k_reset, scancode_to_name(k_reset));
	printf("%s screenshot is %d %s\n", __func__, k_screencap, scancode_to_name(k_screencap));
	printf("%s save is %d %s\n", __func__, k_save, scancode_to_name(k_save));
	printf("%s load is %d %s\n", __func__, k_load, scancode_to_name(k_load));
	#endif
	#endif
	*/
	syskeys[0] = k_quit;
	...
	syskeys[7] = k_inject;
	syskeys[8] = k_swap;
	syskeys[9] = k_load_c7420;
	syskeys[10] = k_save_c7420;
	syskeys[11] = k_copy_c7420;
}
...
void handle_key()
{
	...
	/* QUIT */
	if (key[syskeys[0]] || (key[KEY_ESC] && app_data.c7420 != 1)) {
		do {
			rest(5);
	...
	/* LOAD STATE */
	if (key[syskeys[6]]) {
		if ((stateError = loadstate(app_data.statefile)) == O2EM_SUCCESS)
	...
	if ((key[syskeys[3]]) && ((key[ KEY_LSHIFT])||(key[KEY_RSHIFT]))) {
		init_cpu();
		init_roms();
		init_rams();
		/*
		if (app_data.c7420==1)
			initZ80Memory(app_data.c7420biosfilename);*/
			/* Clear Z80 memory and lines if C7420 is enabled */
		init_vpp();
		clearscr();
		do {
			rest(5);
	...
	if (key[syskeys[7]]) {
		...
		display_msg("Highscore-Data set.", 5);
	}

	/* Activate/deactivate joystick keys */
	if (key[KEY_SCRLOCK]) {
		if (!(((key_shifts & KB_SCROLOCK_FLAG) == 256)^app_data.disableJoyDefault)) {
			display_msg("Joystick on Keys disabled.",5);
		}
		else {
			display_msg("Joystick on Keys enabled.",5);
		}		

		do {
			rest(5);
			if (NeedsPoll) poll_keyboard();
		} while (key[KEY_SCRLOCK]);
 	}
	...
	if (key[syskeys[4]]) {
		if (strlen(app_data.scshot) > 0) {
			...
			fprintf(stderr, "Cant make a screenshot, app_data.scshot is empty\n");
	}
	...
	if (key[syskeys[8]]) {
		...
		display_msg("Joysticks swaped.",5);
	}
